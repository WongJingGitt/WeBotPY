...
class ServiceLLM(Blueprint):
    ...
    def _route_map(self) -> list[dict[str, Union[callable, str]]]:
        return [
            {'rule': '/api/model/model/add', 'view_func': self._add_model, 'methods': ['POST'],
             'endpoint': 'add_model'},
            {'rule': '/api/model/apikey/add', 'view_func': self._add_apikey, 'methods': ['POST'],
             'endpoint': 'add_apikey'},
    ...
    def _get_models(self):
        ...
        try:
            ...
            _response.data = [
                dict(zip(["model_id", "model_name", "model_format_name", "description", "base_url", "apikey_id"], m))
                for m in models]
    ...
    def _update_model(self):
        ...
        if not _request.check_body or _request.body['field'] not in _request.body.get('field') not in ['model_name',
                                                                                                       'model_format_name',
                                                                                                       'base_url',
                                                                                                       'apikey_id',
                                                                                                       'description']:
